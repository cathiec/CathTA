    ...
    bool is_included_in(const basic_tree_automaton & B, int d, basic_set<basic_product_state> * Processed) const
    {
        ...
        else
        {
            ...
            for(int i = 1; i <= DELTA.size(); i++)
                ...
                {
                    if(q_Processed.contain(DELTA[i]._inputs))
                    {
                        basic_set<basic_tuple<basic_set<basic_state> > > possible_P1_to_Pn = possible_combinations_Pi(DELTA[i]._inputs, Processed[d - 1]);
                        for(int j = 1; j <= possible_P1_to_Pn.size(); j++)
