...
public:
    ...
    /// compute the max dimension of the current tree automaton
    container<state> Post(const container<state> Processed, const state qd) const
    {
        container<state> result;
        for(int i = 1; i <= DELTA.size(); i++)
        {
            ...
            bool contains_qd = false;
            int max_d = -1;
            bool max_d_twice = false;
            for(int j = 1; j <= t._input.size(); j++)
            {
                bool one_in_Processed = false;
                for(int k = 1; k <= Processed.size(); k++)
                {
                    if(Processed[k]._name == t._input[j]._name)
                    {
                        one_in_Processed = true;
                        if(Processed[k]._dimension > max_d)
                        {
                            max_d = Processed[k]._dimension;
                            max_d_twice = false;
                        }
                        else if(Processed[k]._dimension == max_d)
                            max_d_twice = true;
                        break;
                    }
                }
                ...
                if(!contains_qd && t._input[j]._name == qd._name)
                    contains_qd = true;
            }
            if(all_in_Processed && contains_qd)
            {
                if(max_d_twice)
                    max_d++;
                state one_result = t._output;
    ...
    {
        int max_D = 0;
        container<state> Processed;
        container<state> Next;
        for(int i = 1; i <= DELTA.size(); i++)
        if(DELTA[i]._alpha._rank == 0)
            Next.add(DELTA[i]._output);
        //int upper_bound = (Q.size() + 1) * (Q.size() + 1);
        int upper_bound = UPPER_BOUND_DIMENSION;
        while(Next.size() > 0)
        {
            state qd = Next[1];
            Next.del(qd);
            Processed.add(qd);
            container<state> Post_Processed_qd = Post(Processed, qd);
            for(int i = 1; i <= Post_Processed_qd.size(); i++)
            {
                state pe = Post_Processed_qd[i];
                if(pe._dimension >= upper_bound)
                    return -1;
                else if(pe._dimension > max_D)
                    max_D = pe._dimension;
                bool exist = false;
                for(int j = 1; j <= Processed.size(); j++)
                {
                    state pf = Processed[j];
                    if(pf._name == pe._name && pe._dimension <= pf._dimension)
                    {
                        exist = true;
                        break;
                    }
