...
def main(file_lhs,file_rhs,verbose,print_vata_files):
	tiles=[]
	...
	(aut1,emptyheap_eq1,eq_edges1)=input.parse_input(file_lhs,tiles,free)
	(aut2,emptyheap_eq2,eq_edges2)=input.parse_input(file_rhs,tiles,free)
	...
	if verbose:
		...
		if print_vata_files:
			print "-----------\nVATA LHS file contains:\n----------"
			os.system("cat %s"%file1)
			print "-----------\nVATA RHS file contains:\n----------"
			os.system("cat %s"%file2)
			print "-----------"
		print "Entailment result:"
	...
	os.system("cp %s /home/cathiec/Desktop/1.cath"%file1)
	os.system("cp %s /home/cathiec/Desktop/2.cath"%file2)
	# remove tmp files
	os.unlink(file1)
	os.unlink(file2)
...
if len(sys.argv)<3:
	...
	print "Print VATA files (same as standard mode + the VATA files are printed): entailment.py -f file_with_pred1 file_with_pred2"
	print "Silent mode (no error messages): entailment.py -s file_with_pred1 file_with_pred2"
	sys.exit()
if sys.argv[1]=="-s":
	try:
		main(sys.argv[2],sys.argv[3],False,False)
	except:
		print "UNKNOWN"
		sys.exit(1)
	
elif sys.argv[1]=="-f":
	main(sys.argv[2],sys.argv[3],True,True)
else:
	main(sys.argv[1],sys.argv[2],True,False)
